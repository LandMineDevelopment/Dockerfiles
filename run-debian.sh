...
if docker container inspect "$CONTAINER_NAME" >/dev/null 2>&1; then
    ...
    echo "Docker command: docker run -it --name $CONTAINER_NAME -v $WORKSPACE_DIR:/workspace -v $HOME/.local/share/debian-dev/usr-local:/usr/local -v $HOME/.local/share/debian-dev/root-local:/root/.local $IMAGE_NAME"
    docker run -it \
      --name "$CONTAINER_NAME" \
